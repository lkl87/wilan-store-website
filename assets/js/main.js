    ...
    if (orderForm) {
        orderForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                let result = null;
                try { result = await res.json(); } catch (parseErr) { /* الرد مش JSON */ }

                if (!res.ok) {
                    const serverMsg = result?.error ? ` (${result.error})` : '';
                    throw new Error(`server_error_${res.status}${serverMsg}`);
                }
                ...
                // نعرض السبب الحقيقي على الشاشة مباشرة (رمز الخطأ من السيرفر أو نوع خطأ الشبكة)
                const detail = err?.message || 'unknown';
                setStatus('error', `<i class="fa-solid fa-circle-exclamation"></i> تعذر إرسال الطلب. تأكدي من اتصال الإنترنت وحاولي مرة أخرى.<br><small style="opacity:.7">تفاصيل تقنية: ${detail}</small>`);
            } finally {
