...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (orderForm) {
        orderForm.addEventListener('submit', async (e) => {
            ...
            try {
                // لو المستخدم مسجل دخول (توكن محفوظ)، نبعته مع الطلب حتى ينربط الطلب فيه تلقائياً
                const wilanToken = localStorage.getItem('wilan_auth_token');
                const fetchHeaders = wilanToken ? { Authorization: `Bearer ${wilanToken}` } : {};
