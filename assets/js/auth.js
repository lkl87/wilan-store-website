...
(function () {
    ...
    // إعادة محاولة تلقائية: لو السيرفر نايم (cold start) أو صار تأخير مؤقت بالشبكة،
    // نعيد المحاولة بدل ما نطلع خطأ للمستخدم من أول فشلة
    function fetchWithRetry(url, options, retries = 3, delayMs = 1500) {
        return fetch(url, options).catch((err) => {
            if (retries <= 0) throw err;
            return new Promise((resolve) => setTimeout(resolve, delayMs))
                .then(() => fetchWithRetry(url, options, retries - 1, delayMs * 1.5));
        });
    }
    ...
    function loadTelegramWidget() {
        ...
        fetchWithRetry(`${API_BASE}/api/bot-username`)
            .then(r => r.json())
            .then(data => {
    ...
    window.__wilanTelegramAuth = function (tgUser) {
        const holder = document.getElementById('wilanTgWidgetHolder');
        if (holder) holder.innerHTML = '<p style="color:#6d7286;font-size:12px;">جاري تسجيل الدخول...</p>';

        fetchWithRetry(`${API_BASE}/api/auth/telegram`, {
            method: 'POST',
            ...
            .catch(() => {
                alert('تعذر الاتصال بالسيرفر لتسجيل الدخول، حاول مرة ثانية بعد شوي');
                loadTelegramWidget();
            });
    };
    ...
    function loadMyOrders() {
        ...
        fetchWithRetry(`${API_BASE}/api/my-orders`, {
